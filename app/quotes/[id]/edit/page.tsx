    ...
    const loadData = async () => {
      try {
        ...
        setForm({
          ...
          discountType: quoteData.discountType || 'fixed',
          discountValue: quoteData.discountValue || '',
          taxRate: quoteData.taxRate || 7.5,
          depositType: quoteData.depositType || 'fixed',
        ...
        // Load and pre-select customer
        const customersRes = await fetch('/api/customers');
        if (customersRes.ok) {
          const customersData = await customersRes.json();
          setCustomers(customersData);

          // Find and select the customer for this quote
          const customer = customersData.find((c: any) => c.id === quoteData.customerId);
          if (customer) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 pb-20 md:pb-0">
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          <Link href="/quotes" className="text-cyan-600 hover:text-cyan-700 mt-4 inline-block">
            Back to Quotes
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <tbody>
                    {items.map((item, idx) => (
                      <tr key={idx} className="border-b">
                        ...
                        <td className="text-right py-2 px-2">${item.pricePerUnit.toFixed(2)}</td>
                        <td className="text-right py-2 px-2">{item.quantity}</td>
                        <td className="text-right py-2 px-2 font-medium">${(item.pricePerUnit * item.quantity).toFixed(2)}</td>
              ...
              <div className="space-y-2 text-sm">
                ...
                {discountAmount > 0 && (
                  <>
                    <div className="flex justify-between py-2 bg-orange-50 px-2 rounded">
                      <span className="text-gray-600">Discount ({form.discountType === 'fixed' ? '$' : discountVal + '%'}):</span>
                ...
                {depositAmountCalculated > 0 && (
                  <div className="flex justify-between py-2">
                    <span className="text-gray-600">Deposit ({form.depositType === 'fixed' ? '$' : depositVal + '%'}):</span>
          ...
          <div className="flex gap-4 justify-end">
            <Link href={`/quotes/${quoteId}`} className="btn-secondary">
              Cancel
            </Link>
